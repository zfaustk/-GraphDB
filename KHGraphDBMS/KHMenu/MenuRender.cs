...
namespace KHGraphDBMS.KHMenu
{
    class MenuRender : ToolStripProfessionalRenderer
    {
        ...
        protected override void OnRenderToolStripBorder(ToolStripRenderEventArgs e)
        {
            ...
            {
                //e.Graphics.DrawLine(new Pen(new SolidBrush(colorconfig.MainBordercolor)), 0, 0, 0, e.ToolStrip.Height);
                //e.Graphics.DrawLine(new Pen(new SolidBrush(colorconfig.MainBordercolor)), e.ToolStrip.Width - 1, 0, e.ToolStrip.Width - 1, e.ToolStrip.Height);
                //e.Graphics.DrawLine(new Pen(new SolidBrush(colorconfig.MainBordercolor)), 0, 0, e.ToolStrip.Width - 1, 0);
        ...
        {
            e.ArrowColor = Color.FromArgb(153,153,153);//设置为红色，当然还可以 画出各种形状  
